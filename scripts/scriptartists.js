...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (loggedInUser) {
        ...
        headerRight.innerHTML = `
            ...
            <a href="profile.html?artist=${encodeURIComponent(loggedInUser)}" class="btn btn-outline">Профиль</a>
            <a href="#" class="btn btn-primary" id="logout-button">Выйти</a>
        `;
        // Re-attach search input event listener after DOM update
        ...
        newThemeToggle.addEventListener("click", () => {
            ...
            newThemeToggle.innerHTML = isDark ? '<i class="fas fa-moon"></i>' : '<i class="fas fa-sun"></i>';
        });
...
function filterArtists() {
    const filteredArtists = artists.filter((artist) =>
        searchQuery === "" || artist.name.toLowerCase().includes(searchQuery) || artist.specialty.toLowerCase().includes(searchQuery)
    );
...
function renderArtists(artists) {
    ...
    artists.forEach((artist, index) => {
        ...
        card.querySelector(".view-profile-button").href = `artist/${artist.profileSlug}.html`;
        const followButton = card.querySelector(".follow-button");
        followButton.textContent = "Follow";
        followButton.addEventListener("click", function () {
            this.classList.toggle("following");
            const followerCount = this.parentNode.parentNode.querySelector(".follower-count");
...
function initScrollObservers() {
    ...
    setTimeout(() => document.querySelectorAll(".artist-card").forEach((card) => observer.observe(card)), 100);
}